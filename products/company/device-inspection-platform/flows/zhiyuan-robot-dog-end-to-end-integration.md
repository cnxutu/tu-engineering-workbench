...
> **证据等级：代码核对已确认（2026-09-08）。** 本文以已落地代码为依据，说明机器狗一个核心能力应如何穿过 P4-1、P3、P2、P1 并回到驾驶舱。它是可复用的集成经验，不替代厂商协议、完整 TSL 或具体部署手册。
...
设备在线性也是独立语义：不能仅凭某个状态字段推断在线，需要按平台的设备会话/在线状态链路核验。当前 P1 在有效状态属性成功处理后会将机器狗的 `online:{deviceSn}` 续期 120 秒；该键不存在时机器狗业务状态归约为 `OFFLINE`，而 P2 的 `DEVICE_OFFLINE` 事件也会删除该键。P4-1 的 `1001` 心跳超时只告警、不直接投递离线事件；完整判定链、超时与证据见 [设备离线状态链路与排查 SOP](device-offline-status-flow.md#智元机器狗的展示状态分支)。
