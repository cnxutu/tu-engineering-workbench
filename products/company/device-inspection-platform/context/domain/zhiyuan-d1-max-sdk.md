# 智元 D1 Max SDK 能力与接入约束

> **证据等级：厂商 SDK 文档核对（V0.1.0，2026-07 勘误版；阅读日期 2026-08-31）。** 本页用于评估智元酷拓四足机器人 D1 Max 接入巡检 IoT 平台时的事实、边界和待确认项。它不是已实现的产品接入设计、物模型或协议契约；目标架构均明确标为 `pending_verification`。

## 何时读取

- 评估 D1 Max 或同系列设备接入、设计 P4-1 智元适配器，或确定 P3 是否需要新协议实例。
...
1. `ProtocolCodecAdapter`：适用于设备已向 P3 协议入口提供原始报文的情形，P3 在上/下行时按协议实例的 `protocolCodecId` 选择 Codec。
2. `CustomProtocolAdapter`：适用于“第三方 SDK 在自身内部建立、维护连接并收发数据”的情形。`IoTCustomGatewayServer` 注册 Adapter 的上行回调；Adapter 可把标准 `UpstreamDecodedData` 回调给 P3，P3 再补齐设备/网关上下文并转交 P2。下行由 P3 调用 Adapter 的 `downstream`。

厂商指南只确认 C++ SDK 的主动连接和回调，不提供可在 Java 中直接重实现的原始通信协议。因此，**不能仅凭该 PDF 决定 P4-1 实现 `ProtocolCodecAdapter`**。以当前证据，更接近的目标接入形态是 P3 的 `CustomProtocolAdapter` 路径；但 Java 对厂商 C++ SDK 的调用方式仍为 `pending_verification`，可能是受支持的 Java/JNI 封装，也可能是独立 C++ bridge 加一份明确的本地/网络契约。gRPC、HTTP/JSON 与 MQTT 的具体取舍见 [P3 与原生 SDK Bridge：gRPC、HTTP 与 MQTT 的选型](grpc-edge-bridge-selection.md)。

```mermaid
sequenceDiagram
    participant SDK as P4-1 目标：智元 SDK Bridge
...
上图是**目标候选**，不是当前部署事实。若 P4-1 是独立 C++ 进程，P3 无法通过 Maven classpath 直接加载它；必须另外定义 bridge 生命周期、认证、反压、断线重连、上/下行消息格式和版本兼容策略。若 P4-1 是可加载的 Java Adapter，则仍须先验证厂商提供受支持的 Java 调用方式及其部署依赖。
...
| 设备身份 | SDK 连接目标与 P2 `productKey`/`deviceName` 如何稳定关联；替换设备后身份如何迁移。 | P2 + P4-1 |
| 运行态 | 上行状态回调、SDK 断连、网络不可达与设备实际不可用分别如何影响 P2 ONLINE/OFFLINE；超时阈值和通道 tags 是什么。 | P2 + P3 + P4-1 |
| 命令完成语义 | 发送成功、机器人确认收到、动作开始、动作完成、动作失败分别由何种回调/状态证明。 | P1 + P2 + P4-1 |
| 安全与控制权 | 角色授权、控制权抢占、软急停、恢复、遥控器人工接管和审计如何定义。 | 产品 + P1 + P2 |
| 数据面 | SDK 回调、ROS2 传感器和 RTSP 视频哪些进入业务消息，哪些走专用媒体/数据管道；高频流的限流、采样和存储策略是什么。 | P1 + P2 + P3 + P4-1 |

当前 `IoTCustomGatewayServer#processDownstream` 在 Adapter 未抛错时以 `NONE` 回复模式返回成功；它本身不等待厂商动作完成。因此，P4-1 不能把“SDK 方法已调用”包装为“巡检动作完成”。在未补充完成事件/状态契约前，P1/P2 应将该结果限定为命令受理或发送结果。

## 最小验证顺序

1. 取得与目标固件匹配的 SDK 压缩包、头文件、示例与许可证，确认可运行环境和依赖；不要仅以 PDF 推断 ABI 或通信协议。
2. 在隔离网络对一台测试机验证连接、断开重连、状态 1 Hz 回调、故障触发/恢复和控制权抢占；记录脱敏样本。
3. 确定 P4-1 的 Java 加载或 C++ bridge 方案，并用 P3 `customs` 协议实例验证上行回调可被 P3 按设备身份查询和投递。
...
6. 准备 TSL、P1 Handler/Data Rule、P3 协议实例和 P4-1 版本的联合回滚，确保新消息不会在 P1 因未知 identifier 被确认消费。

## 证据与未知项

### 厂商 PDF 证据

- `智元四足机器人 D1 Max SDK开发指南V0.1.0_20260729.pdf`：第 24–30 页（开发位置、SDK、环境与连接），第 36–50 页（连接、回调、控制、数据配置与控制权），第 51–58 页（视频和 ROS2 传感器），第 59–67 页（控制权与调试）。
