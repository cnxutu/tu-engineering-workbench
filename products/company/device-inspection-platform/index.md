...
- [系统架构](architecture/index.md)：跨服务职责、基础设施和问题定位边界。
- [流程](flows/index.md)：已核实的端到端消息、状态、指令和设备集成流程。
- [产品决策](decisions/index.md)：长期有效的服务、数据与产品设计边界。
- [仓库知识](repositories/index.md)：按服务和适配器定位代码入口与专题知识。
- [Supporting Context](context/index.md)：设备厂商资料、领域术语与候选技术选型；不替代 Current Product Truth。
- [Runtime Context](runtime/index.md)：开发/测试环境、容器、日志、trace 和运行异常的按需入口。
- [产品 PRD](prd/index.md)：按版本归档的历史需求参考。
