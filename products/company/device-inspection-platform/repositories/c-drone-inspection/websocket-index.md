...
P1 WebSocket 主要面向前端实时刷新和局部状态更新。消息通常通过 `IWebSocketMessageService` 发送到 `/ws/drone` 或按设备类型/设备 SN 路由；全局刷新消息一般只携带“发生变化”的信号，前端收到后重新请求 REST 接口，不能把 WebSocket 当作完整数据快照或可靠消息队列。
