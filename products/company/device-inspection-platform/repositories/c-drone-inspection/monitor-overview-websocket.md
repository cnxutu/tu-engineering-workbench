...
设备空间关联还会在同一事务内发布 `MonitorBusinessOverviewChangedEvent`；事务提交后，业务统计链路广播 `monitor_business_overview_changed`，前端重新请求 `GET /drone/monitor/business-overview`。该协同刷新只适用于设备关联变更，地图标注和电子围栏仍只刷新基础统计；业务统计的完整语义见 [顶部统计刷新 WebSocket 链路](monitor-business-overview-websocket.md)。
...
这些入口在持久化操作成功后发布事件；事务回滚时不会进入正常的提交后监听路径。批量设备操作在一个业务事务内只发布一次基础统计事件和一次业务统计事件。`MonitorOverviewChangedEvent` 的 `operation` 当前使用 `CREATE`、`UPDATE`、`DELETE`、`UNBIND_SPACE` 等字符串，消息消费者不应据此增量修改数字，而应重新拉取概览。
...
- 分别验证标注、电子围栏、设备关联/解绑空间的成功操作；设备关联同时应收到两类统计刷新，事务回滚不应刷新。
