# Engineering Workbench — Living Engineering Model
...
- `core/`：Engineering Operating Model——规则、契约、仓库注册表、Playbook 与最小运行时路由。
- `products/`：Current Product Truth——经证据验证、可长期复用的当前产品架构、能力、边界、链路、领域语义与 ADR。
- `work/`：Delivery Change State——正在发生的 Delivery 完整上下文与已关闭 Delivery 的薄索引。
- `plugins/`：Replaceable execution providers——可触发的 Codex Skill 及其组合能力。
...
- [运行时约束](AGENTS.md)：供 Codex 按范围、可选 Stage Shortcut 和条件读取执行任务的最小入口。
- [文档导航](docs/README.md)：按使用、治理、Codex 与工作流目标查找资料。
- [Workbench 模型](docs/workbench-model.md)：核心区域、Delivery Closing 与 Context Routing 的统一说明。
...
- [Agent Skill 版图](docs/codex/agent-skill-landscape.md)：Skill、Workflow 与 Delivery Authority 边界。
- [项目导航与维护地图](docs/guides/project-guide.md)：供工程师理解 AI 运行时加载链路、工程师使用与维护路径、项目骨架与扩展入口。
- [Core 入口](core/index.md)：角色、规则、Playbook 与契约。
...
- `core/`：跨产品复用的 Engineering Operating Model、角色、规则、Playbook 与契约。
- `products/`：当前已验证的产品事实，不记录形成这些事实的过程。
- `work/`：Delivery Change State；`active/` 保存完整活跃包，`closed/` 保存薄索引，Cold Context 按需访问。
- `plugins/`：少量、边界清晰、可替换的 Codex Skill。
