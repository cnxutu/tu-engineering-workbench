...
需打开、核实或修改已登记源码时，优先读取未提交的 `workspace.local.yaml` 获取本机绝对路径。首次接入且该文件不存在时，可从已提交的 `workspace.example.yaml` 复制创建；文件存在但缺少目标项目条目时，报告缺少的映射并请求路径；条目存在但路径不可访问时，报告配置路径与访问失败，不猜测替代位置。范围外项目仅可作为依赖背景，不读取、核实或修改其源码。K1/K2 在路径登记前不得假定其源码可访问。
