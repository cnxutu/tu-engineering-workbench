...
本文件是 P0-1 作为 Primary（主仓库）时 Codex 的最小运行时入口：先定义**工程范围、工程任务必读基准、任务类型与条件读取、角色边界、指令优先级和事实可信度**，再将公共约束作用于本会话识别出的范围内仓库。Primary 只提供公共协作约束，不自动成为任务的代码阅读、核实或修改范围，也不以 P0-1 文档替代目标仓库的局部约束与代码。本文件不能覆盖 Codex 平台、系统或开发者施加的约束。`README.md`、`docs/` 与产品文档用于人工维护或在下列条件满足时按需读取，不是默认上下文。

## 1. 会话范围与路径地图

当 P0-1 被设置为 Primary 时，Codex 每次任务都应先读取本文件的公共约束，无需用户重复提及 `P0-1`。会话首次任务中，Codex 再从用户的直接任务说明中识别独立项目标记 `P0`、`P0-1`、`P1`–`P7`、`P3-1`、`P4-1`、`P10`、`K1`、`K2`、`K5`、`L1`、`A1`、`S1`；无需固定的范围声明格式。除 Primary 的默认加载外，识别出的标记共同构成本次可操作工程范围，后续消息沿用该范围，直到用户明确变更。引用的示例、代码块、文档标题、路径或历史记录中的项目标记不自动扩大范围；语义不明确时先确认，不猜测扩大可操作范围。

| 标记 | 工程 | 本机路径 |
| --- | --- | --- |
| `P0` | `tu-devkit`：开发环境、CLI、VPS 与工程工具 | 本机工作区映射 |
| `P0-1` | `tu-engineering-workbench`：AI 编排、共享上下文与团队工作流 | 本机工作区映射 |
| `P1` | `c-drone-inspection`：巡检业务 | 本机工作区映射 |
| `P2` | `c-iot-server`：IoT 服务 | 本机工作区映射 |
| `P3` | `c-iot-gateway`：协议与连接入口 | 本机工作区映射 |
| `P3-1` | `c-wvp`：视频流管理平台网关 | 本机工作区映射 |
| `P4` | `ad-iot-codec-adapter-dji`：DJI 编解码适配 | 本机工作区映射 |
| `P4-1` | `ad-iot-codec-adapter-robotDog-zhiyuan`：智元机器狗编解码适配 | 本机工作区映射 |
| `P5` | `c-gateway`：巡检系统统一网关 | 本机工作区映射 |
| `P6` | `c-system`：RBAC 与基础用户信息服务 | 本机工作区映射 |
| `P7` | `c-tag`：树模型与标签资源服务 | 本机工作区映射 |
| `P10` | `tu-device-platform-kernel-lab`：复杂设备场景微内核实验 | 本机工作区映射 |
...
| `S1` | `skills`：外部开源 Agent Skill 项目参考模块 | 本机工作区映射；上游为 `https://github.com/mattpocock/skills` |

项目标记可出现在自然语言、列表、括号或字段中；`P1 + P2`、`P1,P2`、`P1，P2`、`P1、P2`、`P1 P2` 等均识别为 P1 与 P2。短横线 `-` 或连接号 `–` 表示连续范围：`P0–P7` 表示 P0、P1、P2、P3、P4、P5、P6、P7；它不包含 P0-1，可与其他标记混用，例如 `P0-1、P2-P7、P10`。`P0-1`、`P3-1`、`P4-1` 是已登记的独立项目标记，不按范围语法拆分。P8、P9 当前未登记，不能由范围语法推断。`范围：` 只是提高可读性的可选前缀，不是必需格式。

`P0`–`P7`、`P0-1`、`P3-1`、`P4-1`、`P10` 与 `S1` 是 P0-1 中用于会话范围和跨仓库导航的标记；仅 P0-1 文档可用其指代项目。在这些目标仓库及其生成物中，项目标记不得作为项目、服务或仓库名称出现在源码、注释/Javadoc、日志文案、配置、数据库注释、测试名称、接口说明、设计文档、迁移脚本或新生成文件中。生成或修改这些项目的内容时，必须使用实际工程名、服务名、模块名或已有领域术语（例如 `c-drone-inspection`、`c-iot-server`、`skills`）。存量内容仅在当前任务明确要求时迁移；本次迁移只替换注释、备注和文档文字，不得改动代码标识符、协议字段、枚举值、业务数据或运行逻辑。`P1` 等仅表示业务优先级、阶段、分页变量或协议/型号值时，不属于项目代指，可按原语义保留。

例如，用户要求调整 `P1` 的功能时，应先应用 P0-1 的公共约束，再将 P1 作为可操作范围，读取 P1 的局部 `AGENTS.md`（如有）、相关代码与按需上下文；此时 P0-1 不在可修改范围。仅当会话首次任务明确提及 `P0-1` 时，P0-1 才进入可操作范围，并在后续消息中默认沿用。
...
| 维护 P0-1 运行时规则、Core、脚本或校验 | `development.md`、受影响文件和第 3.2 节对应的维护资料；不加载无关产品知识。 |
...
| 条件 | 额外必读项 | 不默认读取 |
| --- | --- | --- |
| 维护 P0-1 的运行时入口、`core/` 公共规则、角色、工作流、契约或模板 | `docs/authoring-guide.md` 的“公共规则维护”，以及受影响文件 | 产品知识、治理规范、使用教程 |
| 维护 P0-1 的产品知识、架构、流程、服务边界、清单或任务记录 | `docs/authoring-guide.md`、`docs/governance.md`，以及受影响的权威页面 | 其他产品目录与所有使用教程 |
| 修改 P0-1 的工具、脚本、校验或团队 Plugin | 目标目录的 README、实现与测试；Plugin 还读取 manifest、相关 `SKILL.md` 与 `tests/test-plugin.sh` | 产品知识、知识编写规范、无关 Plugin |
| 用户明确指定 P0 并明确提出 VPS 部署、升级、调整、网络排查、性能或稳定性问题 | 通过 `workspace.local.yaml` 定位 P0，再读取 P0 的 `docs/vps/AGENTS.md` 和当前链路所需资料；涉及实现时仍读取本仓 `development.md` 与 P0 的 `vps-init/README.md` | 仅因 P0-1 是 Primary、偶然提到 VPS、一般网络问答或无真实 VPS/网络目标的仓库开发，不触发该专项入口 |
...
工具调用若被平台自动审核拒绝，应明确说明这是平台限制，不将其表述为用户拒绝或任务已完成。先判断能否在不改变用户目标的前提下缩小为可安全执行的步骤；不能时，必须向用户发起明确授权申请，而不是因一次拒绝结束、归档或交接任务。授权申请须列明被拒绝的操作与精确对象、审核给出的原因或权限缺口、推荐的继续方案和可选的缩小范围；等待回复期间，任务保持未完成状态。用户授权后，仅重试已获授权的最小操作，并继续原任务。AGENTS 约束不得覆盖平台、系统或开发者的实际权限结果。
