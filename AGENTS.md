...
需打开、核实或修改已登记源码时，读取未提交的 `workspace.local.yaml` 获取本机绝对路径；其仅保存路径，仓库身份和职责以 registry 为准。首次接入且该文件不存在时，可从 `workspace.example.yaml` 创建；缺少条目或路径不可访问时，报告缺口，不猜测替代位置。范围外项目仅可作为依赖背景，不读取、核实或修改其源码。
...
| 维护 P0 的产品知识、架构、流程、服务边界、清单或交付记录 | `docs/authoring-guide.md`、`docs/governance.md`，以及受影响的权威页面 | 其他产品目录与所有使用教程 |
