...
当 P0 被设置为 Primary 时，Codex 每次任务都应先读取本文件的公共约束，无需用户重复提及 `P0`。会话首次任务中，Codex 再从用户的直接任务说明中识别独立项目标记 `P0`–`P7`、`P4-1`、`P10`、`K1`、`K2`、`K5`、`L1`、`A1`、`S1`；无需固定的范围声明格式。除 Primary 的默认加载外，识别出的标记共同构成本次可操作工程范围，后续消息沿用该范围，直到用户明确变更。引用的示例、代码块、文档标题、路径或历史记录中的项目标记不自动扩大范围；语义不明确时先确认，不猜测扩大可操作范围。
...
项目标记可出现在自然语言、列表、括号或字段中；`P1 + P2`、`P1,P2`、`P1，P2`、`P1、P2`、`P1 P2` 等均识别为 P1 与 P2。短横线 `-` 或连接号 `–` 表示连续范围：`P0–P7` 表示 P0、P1、P2、P3、P4、P5、P6、P7；它可与其他标记混用，例如 `P0、P2-P7、P10`。`P3-1`、`P4-1` 是已登记的独立项目标记，不按范围语法拆分。P8、P9 当前未登记，不能由范围语法推断。`范围：` 只是提高可读性的可选前缀，不是必需格式。

`P0`–`P7`、`P3-1`、`P4-1`、`P10` 与 `S1` 是 P0 中用于会话范围和跨仓库导航的标记；仅 P0 文档可用其指代项目。在这些目标仓库及其生成物中，项目标记不得作为项目、服务或仓库名称出现在源码、注释/Javadoc、日志文案、配置、数据库注释、测试名称、接口说明、设计文档、迁移脚本或新生成文件中。生成或修改这些项目的内容时，必须使用实际工程名、服务名、模块名或已有领域术语（例如 `c-drone-inspection`、`c-iot-server`、`skills`）。存量内容仅在当前任务明确要求时迁移；本次迁移只替换注释、备注和文档文字，不得改动代码标识符、协议字段、枚举值、业务数据或运行逻辑。`P1` 等仅表示业务优先级、阶段、分页变量或协议/型号值时，不属于项目代指，可按原语义保留。
