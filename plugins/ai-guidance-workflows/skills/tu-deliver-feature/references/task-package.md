...
New Deliveries create only `task.yaml`, `resume.md`, and the current phase Artifact; Artifact keys point only to files that already exist. Add Contract, OpenAPI, Backlog, and Integration keys as their files are created. A Task Loop Verify only updates its parent Artifact. A separate explicit Product Truth Sync may maintain independently established Current Truth but never this Delivery metadata. When the whole Delivery reaches G4 / acceptance or another Closing condition, `tu-deliver-feature` only reports readiness and suggests explicit user invocation of `tu-close-delivery`; it does not mutate Closing state. The Closing Skill compares the Delivery's final verified reusable facts with current canonical Product Truth, without tracking Sync history; it makes Current Truth correct without duplicate conclusions, resolves `knowledge_update_assessment` to `updated` or `not-needed` as the Delivery-level outcome, performs Sensitive Data Review, resolves the local configured `tu-vault` path with the shared `delivery_archive_root` from `core/registry/external-contexts.yaml`, archives the full package to exactly `<delivery_archive_root>/<delivery-id>/`, finalizes and verifies the archived snapshot, creates `work/closed/<domain>/<product>/<delivery-id>.md`, and retires the active Package. Retained `work/<domain>/<product>/tasks/archive/` remains legacy/local cold compatibility only. `blocked` and `superseded` can also close; Archive means closed lifecycle/cold context, not successful delivery.
...
Closed Index status is `completed`, `blocked`, or `superseded`; `superseded` must include a replacement DF distinct from the current Delivery in `Related Deliveries` or `Superseded By`. `closed` describes the lifecycle location, never a Status value. When a later Bug belongs to a closed Delivery, resolve its configured external archive reference first; if unavailable, inspect retained local legacy cold history as a compatibility fallback. Restore/copy only immutable `<Archive Unit>/delivery/` as `work/active/<domain>/<product>/<DF-ID>-<slug>/`; never copy the Unit's `summary.md` or `manifest.yaml`, move or delete the external Archive Unit, or modify its archived `delivery/task.yaml`. Preserve `delivery_id`, recover a reliable archived slug when available or derive it from title, set `status: active`, reset only the active copy's `knowledge_update_assessment: deferred` regardless of the archived `updated` or `not-needed` outcome, remove `archived_at`, `archive_reference`, and `closed_index`, and record Reopened At, Reason, Previous Closing Status, Previous Closed At, Archive Reference, related BUG/CAP, and Current Classification in `04-integration-log.md`. If neither external archive nor local legacy history is available, report the evidence gap instead of inventing context. Refresh the resume cache before continuing.
