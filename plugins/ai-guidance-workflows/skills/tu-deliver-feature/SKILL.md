...
Before closing a Gate, verify its stated evidence. After every meaningful update, change the current Artifact, `task.yaml`, and `resume.md` together enough that a fresh Session can continue without replaying chat history. A DEV / BUG / INT Task Loop Verify only updates its parent Artifact; it does not close the Delivery. A separate, explicit Product Truth Sync may independently maintain already-established Current Truth, but it does not mutate this Delivery. When G4 / acceptance or another Closing condition is met, report **Delivery Closing Readiness: ready** and suggest `tu-close-delivery DF-YYYYMMDD-NN`; do not perform Delivery final Product Truth reconciliation, Archive, create a Closed Index, or retire the Active Package without that Skill's explicit user authorization.
