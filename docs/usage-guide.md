...
会话首次任务中出现 `P0`–`P4`、`K1`、`K2`、`K5`、`A1` 等项目标记时，Codex 应直接识别本次范围；以下 `范围：` 只是便于工程师阅读的写法：
...
`P0–P4` 是当前无人机巡检系统的全量范围；`K1` 和 `K2` 分别预留给 Knowledge Hub 后端与前端；`K5` 是语言学习与快速 Demo 运行项目 `tu-language-lab`；`A1` 是个人 AI 功能集合工程 `tu-ai-lab`。本机绝对路径维护在未提交的 `workspace.local.yaml`；首次接入时从 [`../workspace.example.yaml`](../workspace.example.yaml) 复制创建。缺少映射时不得猜测源码位置。
