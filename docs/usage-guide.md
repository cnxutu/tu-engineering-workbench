...
会话首次任务中出现 `P0`–`P7`、`K1`、`K2`、`K5`、`L1`、`A1` 等项目标记时，Codex 应直接识别本次范围；以下 `范围：` 只是便于工程师阅读的写法：
...
`P0–P7` 是当前无人机巡检系统的全量范围；`K1` 和 `K2` 分别预留给 Knowledge Hub 后端与前端；`K5` 是语言学习与快速 Demo 运行项目 `tu-language-lab`；`L1` 是个人软考高级系统架构师学习沉淀汇总项目 `tu-arch-learning`；`A1` 是个人 AI 功能集合工程 `tu-ai-lab`。本机绝对路径维护在未提交的 `workspace.local.yaml`；首次接入时从 [`../workspace.example.yaml`](../workspace.example.yaml) 复制创建。缺少映射时不得猜测源码位置。P0 之外的项目文件不得把 `P1`–`P7` 当作项目或服务名称，应改用实际工程名；优先级、阶段、变量名和协议/型号值等非项目语义不受此限制。
