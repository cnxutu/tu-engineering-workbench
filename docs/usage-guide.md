...
| K1 | `knowledge-hub`：Knowledge Hub 后端项目（本机路径待登记） |
| K2 | `knowledge-web`：Knowledge Hub 前端项目（本机路径待登记） |
...
建议在会话首次任务中显式声明范围，例如 `范围：P0 + P1` 或 `范围：P0–P4`。后续任务沿用该范围，直到另行声明；范围外仓库仅作为产品知识中的依赖背景，不读取或修改源码。`K1` 和 `K2` 的本机代码位置确定后，登记到 `workspace.yaml` 后才可作为源码范围使用。
