...
Authority 边界：`docs/` 是人类最佳实践；`plugins/**/SKILL.md` 与 references 是 Runtime Skill Contract；`work/active` 是 Change in Progress，`work/closed` 是 Historical Context Index；`products/` 是 Current Verified Product Truth；`core/` 是稳定规则与契约。发生冲突时遵循现有 instruction hierarchy。
