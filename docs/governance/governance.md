...
P0 `tu-engineering-workbench` 是共享工程知识库。只有读者能区分已验证事实与假设，并能追溯事实证据时，它才有价值。

知识库维护的是关键入口、链路和设计边界；评审新增内容时，同时检查它是否可长期复用、是否足够精炼、是否已有权威页面。不能满足任一项的细节应放入代码、契约、配置或 `work/` 交付记录，而非新增产品文档。仓库身份与产品绑定由 registry 维护；仓库责任与已验证产品知识由产品 repository manifest 维护。
