...
flowchart TD
    ...
    EXECUTION --> DEV_TASK[DEV tasks<br/>Explore → Plan → Execute → Verify]
    ...
    SENSITIVE --> ARCHIVE[Archive Full History to tu-vault]
...
Delivery Closing 对整个 Delivery 的最终 Product Truth Delta 做 reconciliation：比较最终实现、契约和证据与当前 canonical `products/` 页面，不追踪此前 `S` action。已正确表达的结论不重复写入；后续工作改变或失效的事实修正、删除或替换；最终尚未表达的事实最小补齐。只将已验证、仍有效且可复用的能力、链路、契约、约束或 ADR 更新至 `products/` 的唯一权威页。`knowledge_update_assessment` 是 Delivery-level outcome：存在 current-worthy Delta 且 Current Product Truth 已正确表达时为 `updated`，即使 Closing 本轮没有文档 diff；最终没有 current-worthy Delta 时才为 `not-needed`。Requirement Authority 来自 Current Product Spec/PRD、approved Product Decision、approved Contract 或明确 acceptance criteria；Implementation Reality 来自代码、测试、配置与可复现运行证据。两者不一致时记录 Requirement / Implementation Gap，不能让代码反向否定已批准需求。对于 `superseded`，不得把被替代的旧设计写成 Current Product Truth，并应记录 related delivery / superseded-by reference。
