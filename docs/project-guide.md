...
| 明确涉及 P1–P4 的 API、消息、数据归属、协议、MQTT、OSD、上下行链路或跨服务发布 | 产品 `index.md`，再沿链接读取当前已维护的最小 Flow 或 P1 入口/缓存资料；未覆盖场景以受影响仓库的局部约束、代码、契约和配置核实 | 整个产品目录、未受影响服务的源码 | “P1 调用 P2 的新指令需要由 P3/P4 编码并发布。” |
| 仅写了多个项目标记，但未说明交互边界 | 已明确范围内项目各自的局部约束；服务关系不明时先确认 | 不自动加载 P1–P4 全局上下文 | “P1、P2 帮我看看这个问题。” |
