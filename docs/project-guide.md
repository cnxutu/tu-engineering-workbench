...
| 明确涉及 P1–P4 的 API、消息、数据归属、协议、MQTT、OSD、上下行链路或跨服务发布 | 产品 `index.md`，再沿链接读取最小必要的架构、Flow、代码入口或缓存资料；每个受影响仓库的局部约束和目标代码 | 整个产品目录、未受影响服务的源码 | “P1 调用 P2 的新指令需要由 P3/P4 编码并发布。” |
