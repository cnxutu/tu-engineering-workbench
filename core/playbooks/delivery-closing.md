...
```text
Inspect → Closing Assessment → Integrate Product Truth → Sensitive Data Review
...
7. **Retire Active Package**：仅在 Integrate、Sensitive Data Review、Archive、Archive Verification 与 Closed Index 全部成功后，才移除对应 active Package。任何中间失败都保留 active Package。
